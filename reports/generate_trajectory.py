...
def load_map_from_pcd(pcd_path: Path) -> tuple[np.ndarray, str]:
    """Load a PCD file and return (Nx3 points array, frame_id)."""
    import open3d as o3d

    pcd = o3d.io.read_point_cloud(str(pcd_path))
    points = np.asarray(pcd.points)
    if points.shape[0] == 0:
        raise ValueError(f"no points in PCD file: {pcd_path}")
    if points.shape[1] < 3:
        raise ValueError(f"expected xyz columns, got shape {points.shape}")
    return points[:, :3], "map"
...
def main() -> int:
    ...
    parser.add_argument("--map-pcd-file", type=Path, default=None, help="PCD file to use as map overlay instead of bag topic.")
    parser.add_argument("--no-bag-map", action="store_true", help="Skip loading map from bag even when --bag-path is set.")
    ...
    map_points = None
    if args.map_pcd_file is not None:
        map_points, map_frame_id = load_map_from_pcd(args.map_pcd_file)
        print(f"map_points={len(map_points)} (from PCD)")
        print(f"map_frame_id={map_frame_id}")
    elif args.bag_path is not None and not args.no_bag_map:
